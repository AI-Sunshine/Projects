...
def totals_tree(m):
    ...
    if is_planet(m):
        planet_size = size(m)
        return tree(planet_size)
    branch = [totals_tree(end(single(m))) for single in [left, right]]
    return tree(sum([label(one) for one in branch]), branch)
...
def replace_leaf(t, find_value, replace_value):
    ...
    return tree(label(t), [replace_leaf(other, find_value, replace_value) for other in branches(t)])
...
def has_path(t, word):
    ...
    if len(word) == 1:
        return True
    for one in branches(t):
        if has_path(one, word[1:]):
            return True
    else:
        return False            
